...
#
# Output is GFM markdown (renders as a real table in Copilot Chat), printed to stdout
# and saved to .context/outline-<name>.md so it survives after the chat scrolls.
#
# Usage:
...
# Escape a literal pipe so a signature can never break a markdown table row.
esc() { printf '%s' "$1" | sed 's/|/\\|/g'; }

FIRST_NAME="$(basename "$1" .java)"
if [ $# -eq 1 ]; then
  OUT_FILE="${OUT_DIR}/outline-${FIRST_NAME}.md"
else
  OUT_FILE="${OUT_DIR}/outline-${FIRST_NAME}-and-$(($# - 1))-more.md"
fi
    ...
    awk -v esc_cmd="cat" '
      ...
      line ~ /\/\*/ && line !~ /\*\// { in_comment = 1; next }
      {
        ...
        if (in_method && depth <= 1 && closes > 0) {
          printf "| method | %s (%sL) | `%s` |\n", start "-" NR, (NR - start + 1), sig
          in_method = 0
        }
      }
    ' "$FILE"
